...
1. 血DK输出 (Blood DPS)：
   - 严格技能序列：疫病维护 > 心脏打击 > 死亡打击 > 死亡缠绕
   - 优化符文之舞爆发窗口
   - 完善开怪序列

2. 血DK坦克 (Blood Tank)：
   - 新增专用坦克APL
   - 紧急自保优先级：生命值<50%时优先灵界打击
   - AOE循环优化：死亡凋零 > 血沸 > 心脏打击

3. 冰霜DK (Frost DPS)：
   - 血系/邪恶系两套APL
   - 核心循环：湮灭 > 冰霜打击 > 冰冷之嚎(触发)
   - 坚不可摧护甲爆发窗口优化
...
1. 修复APL表达式解析问题（or/and关键字）
2. 修复action.xxx.lastcast属性支持
3. 萨满漩涡武器ID添加（泰坦服熔岩武器）
...
3. 使用说明：
   - 输入中文名称进行模糊搜索（如"冰"会匹配所有包含"冰"的技能）
   - 输入英文key进行精确搜索（如"frost"会匹配所有包含"frost"的技能key）
   - 留空搜索框显示所有技能/装备
...
1. 事件处理优化：
   - 新增事件节流系统，减少高频事件的CPU占用
   - UNIT_AURA节流50ms，UNIT_POWER_UPDATE节流30ms
   - 帧更新节流至约33fps，保持流畅的同时降低CPU占用

2. 优化：
   - 新增推荐技能缓存系统，50ms缓存有效期
   - 降低更新频率从150ms到100ms
   - 版本更新至3.80.0-1.0.4-泰坦优化版
...
1. 资源预测优化：
   - 动态预测时长：根据急速自动调整预测时间（12-20秒）
   - 高急速场景下延长预测窗口，确保资源计算准确
...
3. 光环采集优化：
   - 新增增量更新机制，只在光环变化时重新扫描
   - 使用哈希值快速检测光环变化，减少CPU占用
...
6. 预读施法优化：
   - 高急速场景下，施法即将结束时提前计算下一个技能
   - 可配置预读阈值（默认0.3秒）

7. 配置界面：
   - 在"快照"设置页面新增"泰坦重铸版优化"配置项
   - 可调整：资源预测时长、最大事件数量、GCD下限、预读阈值等
